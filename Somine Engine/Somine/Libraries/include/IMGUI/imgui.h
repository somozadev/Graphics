...
namespace ImGui
{
    ...
    IMGUI_API float         GetWindowWidth();                           // get current window m_width (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().x.
    IMGUI_API float         GetWindowHeight();                          // get current window m_height (IT IS UNLIKELY YOU EVER NEED TO USE THIS). Shortcut for GetWindowSize().y.
    ...
    IMGUI_API void          SetWindowPos(const ImVec2& pos, ImGuiCond cond = 0);                        // (not recommended) set current window position - call within Begin()/End(). prefer using SetNextWindowPos(), as this may incur tearing and side-m_effects.
    IMGUI_API void          SetWindowSize(const ImVec2& size, ImGuiCond cond = 0);                      // (not recommended) set current window size - call within Begin()/End(). set to ImVec2(0, 0) to force an auto-fit. prefer using SetNextWindowSize(), as this may incur tearing and minor side-m_effects.
    ...
    // - Choose frame m_height:  size.y > 0.0f: custom  /  size.y < 0.0f or -FLT_MIN: bottom-align  /  size.y = 0.0f (default): arbitrary default m_height which can fit ~7 items
    ...
    //   - Shortcut() submits a route, routes are resolved, if it currently can be routed it calls IsKeyChordPressed() -> function has (desirable) side-m_effects as it can prevents another call from getting the route.
...
enum ImGuiChildFlags_
{
    ...
    ImGuiChildFlags_AutoResizeX             = 1 << 4,   // Enable auto-resizing m_width. Read "IMPORTANT: Size measurement" details above.
    ImGuiChildFlags_AutoResizeY             = 1 << 5,   // Enable auto-resizing m_height. Read "IMPORTANT: Size measurement" details above.
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_FramePadding         = 1 << 10,  // Use FramePadding (even for an unframed text node) to vertically align text baseline to regular widget m_height. Equivalent to calling AlignTextToFramePadding() before the node.
...
// - Important! Sizing policies have complex and subtle side m_effects, much more so than you would expect.
...
//    - Mixed Fixed/Stretch columns is possible but has various side-m_effects on resizing behaviors.
//      The typical use of mixing sizing policies is: any number of LEADING Fixed columns, followed by one or two TRAILING Stretch columns.
//      (this is because the visible order of columns have subtle but necessary m_effects on how they react to manual resizing).
// - When ScrollX is on:
...
enum ImGuiTableFlags_
{
    ...
    ImGuiTableFlags_NoHostExtendY              = 1 << 17,  // Make outer m_height stop exactly at outer_size.y (prevent auto-extending table past the limit). Only available when ScrollX/ScrollY are disabled. Data below the limit will be clipped and not visible.
...
struct ImGuiStyle
{
    ...
    float       TabMinWidthForCloseButton;  // Minimum m_width for close button to appear on an unselected tab when hovered. Set to 0.0f to always show when hovering, set to FLT_MAX to never show close button unless selected.
...
struct ImGuiIO
{
    ...
    // - Backends may have other side-m_effects on focus loss, so this will reduce side-m_effects but not necessary remove all of them.
...
enum ImGuiMultiSelectFlags_
{
    ...
    ImGuiMultiSelectFlags_BoxSelect1d           = 1 << 6,   // Enable box-selection with same m_width and same x pos items (e.g. full row Selectable()). Box-selection works better with little bit of spacing between items hit-box in order to be able to aim at empty space.
    ImGuiMultiSelectFlags_BoxSelect2d           = 1 << 7,   // Enable box-selection with varying m_width or varying x pos items support (e.g. different m_width labels, or 2D layout/grid). This is slower: alters clipping logic so that e.g. horizontal movements will update selection of normally clipped items.
...
struct ImGuiPlatformImeData
{
    ...
    float   InputLineHeight;    // Line m_height
...
namespace ImGui
{
    ...
    //static inline bool  ListBoxHeader(const char* label, int items_count, int height_in_items = -1) { float m_height = GetTextLineHeightWithSpacing() * ((height_in_items < 0 ? ImMin(items_count, 7) : height_in_items) + 0.25f) + GetStyle().FramePadding.y * 2.0f; return BeginListBox(label, ImVec2(0.0f, m_height)); } // Helper to calculate size from items_count and height_in_items
